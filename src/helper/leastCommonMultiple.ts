    ...
    numbers.forEach((numberToFactorize) => {
        ...
        factors.forEach((factor) => {
            primeFactorsAndCount[factor] =
                (primeFactorsAndCount[factor] ?? 0) > 0 ? primeFactorsAndCount[factor] + 1 : 1;
        });

        for (const factor in primeFactorsAndCount) {
            if ((primesToCalculateLCM[factor] ?? 0) > 0) {
                primesToCalculateLCM[factor] = Math.max(primesToCalculateLCM[factor], primeFactorsAndCount[factor]);
            } else {
