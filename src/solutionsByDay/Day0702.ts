...
function determineHandCategory(handAndBid: HandAndBid): void {
    ...
    const maxCountOfSameCardsWithoutJoker = Object.entries(countByChar)
        .filter(([key]) => typeof key === "string" && key !== "J")
        .reduce((max, [, count]) => Math.max(max, count), 0);

    const jokerCount = countByChar.J ?? 0;

    if (maxCountOfSameCardsWithoutJoker + jokerCount === 5) {
        fiveOfAKindCategory.push(handAndBid);
    } else if (maxCountOfSameCardsWithoutJoker + jokerCount === 4) {
        ...
        (Object.values(countByChar).filter((count) => count === 2).length === 2 && jokerCount === 1)
    ) {
        fullHouseCategory.push(handAndBid);
    } else if (maxCountOfSameCardsWithoutJoker + jokerCount === 3) {
    ...
    } else if (maxCountOfSameCardsWithoutJoker + jokerCount === 2) {
...
function cardHandComparer(cardHandA: string, cardHandB: string): number {
    const cardOrderFromHighestToLowest = "AKQT98765432J";
