...
export async function runSolution(): Promise<number> {
    ...
    lines.forEach((line, index) => {
        ...
        for (let i = 0; i < line.length; i++) {
            if (rockIndicesByColumn[i] === undefined || rockIndicesByColumn[i] === null) {
            ...
            if (line[i] === cubeRockChar) rockIndicesByColumn[i].cubeRocksRowIndex.push(index);
            if (line[i] === roundRockChar) rockIndicesByColumn[i].roundRocksRowIndex.push(index);
        }
    });
    ...
    for (const column in rockIndicesByColumn) {
        const { cubeRocksRowIndex, roundRocksRowIndex } = rockIndicesByColumn[column];
        ...
        for (let i = 0; i < cubeRocksRowIndex.length - 1; i++) {
            const roundRocksInBetween = roundRocksRowIndex.filter(
                (roundRockRowIndex) =>
                    roundRockRowIndex > cubeRocksRowIndex[i] && roundRockRowIndex < cubeRocksRowIndex[i + 1],
            );
